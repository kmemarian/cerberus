...
let cerberus debug_level progress core_obj
  ...
  let runM = function
    | Error (loc, Errors.(DESUGAR (Desugar_UndefinedBehaviour ub))) when (batch = `Batch || batch = `CharonBatch || batch = `JsonBatch) ->
        ...
        epilogue 1
    | Error (loc, Errors.(AIL_TYPING (TypingError.TError_UndefinedBehaviour ub))) when (batch = `Batch || batch = `CharonBatch || batch = `JsonBatch) ->
        let open Driver_ocaml in
        print_string begin
          string_of_batch_output ~json:(batch = `JsonBatch) ~is_charon:(batch = `CharonBatch) None
            ([], Undefined { ub; stderr= ""; loc })
