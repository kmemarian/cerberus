...
val loadedValueFromMemValue: Impl_mem.mem_value -> core_object_type * loaded_value
...
val mk_neg_pe: Ctype.integerType -> pexpr -> pexpr
...
val mk_std_undef_pe: Cerb_location.t -> string -> Undefined.undefined_behaviour -> pexpr
...
val mk_save_e_: Annot.annot list -> (Symbol.sym * core_base_type) -> (Symbol.sym * ((core_base_type * (Ctype.ctype * pass_by_value_or_pointer) option) * pexpr)) list -> unit expr -> unit expr
...
val pcreate_readonly: Cerb_location.t -> pexpr -> pexpr -> pexpr -> Symbol.prefix -> unit expr
val pkill: Cerb_location.t -> kill_kind -> pexpr -> unit expr
val pstore: Cerb_location.t -> pexpr -> pexpr -> pexpr -> Cmm_csem.memory_order -> unit expr
val pstore_lock: Cerb_location.t -> pexpr -> pexpr -> pexpr -> Cmm_csem.memory_order -> unit expr
val pload: Cerb_location.t -> pexpr -> pexpr -> Cmm_csem.memory_order -> unit expr
val pcompare_exchange_strong: Cerb_location.t -> pexpr -> pexpr -> pexpr -> pexpr -> Cmm_csem.memory_order -> Cmm_csem.memory_order -> unit expr
val pcompare_exchange_weak: Cerb_location.t -> pexpr -> pexpr -> pexpr -> pexpr -> Cmm_csem.memory_order -> Cmm_csem.memory_order -> unit expr
val plinux_load: Cerb_location.t -> pexpr -> pexpr -> Linux.linux_memory_order -> unit expr
val plinux_store: Cerb_location.t -> pexpr -> pexpr -> pexpr -> Linux.linux_memory_order -> unit expr
val plinux_rmw: Cerb_location.t -> pexpr -> pexpr -> pexpr -> Linux.linux_memory_order -> unit expr

val seq_rmw:
  Cerb_location.t -> bool ->
...
val select_case: (Symbol.sym -> value -> 'a -> 'a) -> value -> (pattern * 'a) list -> 'a option
...
val update_env: pattern -> value -> ((Symbol.sym, value) Pmap.map) list -> ((Symbol.sym, value) Pmap.map) list


val find_labeled_continuation: Symbol.sym -> 'a typed_expr -> (Symbol.sym list * 'a typed_expr) option
val collect_labeled_continuations_NEW: 'a file -> (Symbol.sym, (Symbol.sym, ((Symbol.sym * core_base_type) list * 'a expr)) Pmap.map) Pmap.map


(* ONLY USED by millicore.ml *)
type 'a m_labeled_continuation =
  ( core_base_type
  * (Symbol.sym * ((core_base_type * (Ctype.ctype * pass_by_value_or_pointer) option) * pexpr)) list
  * 'a expr
  * Annot.annot list )
